---
title: "Слова Славы"
description: "Личный сайт Славы про продуктовый и проектный менеджмент (Product Management и Project Management), AI и прикладные системные заметки"
tags:
  - home
  - personal
draft: false
aliases:
  - /about
---

# Слова Славы

Я строю продукты на стыке **AI, бизнеса и системного мышления**. Этот сайт - моя рабочая база знаний: не витрина, а практичный набор материалов, к которым я возвращаюсь сам.

## Кто я

- Senior PM (старший продакт-менеджер) в AI-направлениях.
- Ко-фаундер и оператор команд, где нужно быстро переводить хаос в понятную систему.
- Фокус: проверка спроса, быстрые эксперименты, продуктовая аналитика и запуск.

## Маршруты чтения

### 1. Продуктовый менеджмент (Product Management)

Стартовая точка: [[pm-study|Шпаргалка по продуктовому менеджменту (Product Management Cheatsheet)]]

Ключевые разделы:

- [[knowledge/product-management/03-idea-discovery-and-validation|Поиск идей и валидация]]
- [[knowledge/product-management/04-market-research|Анализ рынка]]
- [[knowledge/product-management/07-strategy-creation|Создание стратегии]]
- [[knowledge/product-management/06-product-analytics|Продуктовая аналитика]]

### 2. Проектный менеджмент (Project Management)

Стартовая точка: [[project-management-study|Шпаргалка по проектному менеджменту (Project Management Cheatsheet)]]

Ключевые разделы:

- [[knowledge/project-management/03-creating-okrs|Построение OKR для проекта]]
- [[knowledge/project-management/12-critical-path|Построение критического пути]]
- [[knowledge/project-management/19-quality-management|Ключевые концепции управления качеством]]
- [[knowledge/project-management/22-scrum-guide-2020|Руководство по Scrum 2020]]

### 3. Агентик инжиниринг (Agentic AI)

Стартовая точка: [[agentic-engineering/index|Гайд по агентик инжинирингу: от новичка до продвинутого]]

Ключевые материалы:

- [[agentic-engineering/wtf-is-loops|Что за «loops», о которых все говорят]]

## Структурированные хабы

- [[knowledge/product-management/index|База знаний по продуктовому менеджменту]]
- [[knowledge/project-management/index|База знаний по проектному менеджменту]]

## Контакты

- Telegram: [@sslava_g](https://t.me/sslava_g)
- GitHub: [Slavikss](https://github.com/Slavikss)
- LinkedIn: [vguch](https://linkedin.com/in/vguch)
